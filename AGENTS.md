...
`go:generate` directive and written to `cmd/dum/installer.schema.json`,
where it is embedded into the binary. `make generate` regenerates it;
commit the updated file when task types or config fields change.
