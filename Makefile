...
.DEFAULT_GOAL := help
DISPATCH = OUT_DIR="$(OUT_DIR)" DUM_VERSION="$(DUM_VERSION)" COVERAGE_THRESHOLD="$(COVERAGE_THRESHOLD)" DUM_APP="$(DUM_APP)" DUM_SOURCE="$(DUM_SOURCE)" GIT_COMMIT="$(GIT_COMMIT)" ./tools/build.sh
